...
impl Row {
    ...
    #[must_use] pub const fn new(values: Vec<crate::core::common::types::Value>) -> Self {
        Self { values }
    }
    
    /// Gets a value by column index
    #[must_use] pub fn get(&self, index: usize) -> Option<&crate::core::common::types::Value> {
        self.values.get(index)
    }
    
    /// Gets a value by column name (requires column metadata from `QueryResultData`)
    #[must_use] pub fn get_by_name(&self, columns: &[String], name: &str) -> Option<&crate::core::common::types::Value> {
        columns.iter()
...
impl QueryResult {
                    ...
                    if is_similarity_search {
                        ...
                        let rows: Vec<Row> = ranked_results.into_iter()
                            .map(|(distance, data_types)| {
                                ...
                                values.insert(0, crate::core::common::types::Value::Float(f64::from(distance)));
                                Row::new(values)
                            })
                            .collect();
    ...
    #[must_use] pub const fn data(&self) -> Option<&QueryResultData> {
        match self {
...
impl QueryResultData {
    ...
    #[must_use] pub const fn new(columns: Vec<String>, rows: Vec<Row>) -> Self {
        Self { columns, rows }
    }
    ...
    #[must_use] pub fn get_row(&self, index: usize) -> Option<&Row> {
        self.rows.get(index)
    }
    ...
    #[must_use] pub fn columns(&self) -> &[String] {
        &self.columns
    }
}
