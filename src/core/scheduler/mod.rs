...
impl Scheduler {
    ...
    pub fn get_stats(&self) -> Result<SchedulerStats, String> {
        ...
        let mut stats = SchedulerStats { uptime: self.start_time.elapsed(), ..Default::default() };
            ...
            match &task.status {
                ...
                TaskStatus::Completed => {
                    ...
                    if let Some(completed_at) = task.completed_at {
                        if last_completed.is_none_or(|last| completed_at > last) {
    ...
    pub fn get_tasks_by_priority(&self, priority: Priority) -> Result<Vec<Task>, String> {
        ...
        let filtered_tasks: Vec<Task> =
            tasks.values().filter(|task| task.priority == priority).cloned().collect();
    ...
    pub fn cleanup_old_tasks(&self, older_than: Duration) -> Result<u64, String> {
        let mut tasks = self.tasks.lock().map_err(|e| format!("Lock error: {e}"))?;
        let cutoff_time = Instant::now().checked_sub(older_than).ok_or_else(|| {
            format!("'older_than' duration ({older_than:?}) is too large and causes an underflow")
        })?;
        ...
        let task_ids_to_remove: Vec<u64> = tasks
            ...
            .filter(|(_, task)| {
                ...
                ) && task.completed_at.is_some_and(|completed_at| completed_at < cutoff_time)
            })
            .map(|(id, _)| *id)
            .collect();
...
mod tests {
    ...
    fn test_task_cancellation() {
        ...
        let task_id =
            scheduler.schedule_task("Cancelled Task".to_string(), Priority::Critical).unwrap();
