    ...
    fn test_invalid_character() {
        ...
        if let Err(ref e) = result {
            if !matches!(e, SqlTokenizerError::InvalidCharacter('#', 34)) {
                assert!(false, "Tokenizer error mismatch. Expected InvalidCharacter('#', 34), got {:?}", e);
            }
        } else {
            assert!(false, "Tokenizer succeeded, but expected InvalidCharacter error. Got: {:?}", result);
        }
