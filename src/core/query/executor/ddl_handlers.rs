...
impl<S: KeyValueStore<Vec<u8>, Vec<u8>>> QueryExecutor<S> {
    pub(crate) fn handle_insert(
        ...
        if let Some(active_tx_mut) = self.transaction_manager.get_active_transaction_mut() {
            ...
            let current_value = self.store.read().unwrap().get(&key, active_tx_id, &committed_ids_snapshot)?;
            let undo_op = if let Some(old_val) = current_value {
                UndoOperation::RevertUpdate { key: key.clone(), old_value: old_val } // Or a specific RevertOverwrite
            } else {
            ...
            let put_result = self.store.write().unwrap().put(key.clone(), serialized_value.clone(), &tx_for_store);
    ...
    pub(crate) fn handle_delete(
        ...
        let old_value_opt = self.store.read().unwrap().get(&key, current_operation_tx_id, &committed_ids_snapshot_for_get)?;
        ...
        for primary_key in candidate_keys {
            ...
            match self.store.read().unwrap().get(&primary_key, snapshot_id, &committed_ids) {
                Ok(Some(serialized_data_from_store)) => {
                    ...
                    if serialized_data_from_store == value { // This comparison logic might need adjustment based on what the index actually stores
