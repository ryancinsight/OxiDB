...
use crate::core::storage::engine::traits::KeyValueStore; // Required for TransactionId(0)
...
impl<S: KeyValueStore<Vec<u8>, Vec<u8>> + Send + Sync + 'static> QueryExecutor<S> {
    pub fn execute_command(&mut self, command: Command) -> Result<ExecutionResult, OxidbError> {
        ...
        let is_transaction_management_command = matches!(
            command,
            Command::BeginTransaction | Command::CommitTransaction | Command::RollbackTransaction
        );

        if !is_transaction_management_command
            && self.transaction_manager.current_active_transaction_id().is_none()
        {
