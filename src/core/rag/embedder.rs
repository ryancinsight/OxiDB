...
impl TfIdfEmbedder {
    ...
    pub fn new(documents: &[Document]) -> Self {
        ...
        Self { vocabulary, idf_scores, dimension }
    }
...
impl EmbeddingModel for TfIdfEmbedder {
    async fn embed_document(&self, document: &Document) -> Result<Embedding, OxidbError> {
        ...
        for (word, tf) in tf_map {
            if let (Some(&vocab_idx), Some(&idf)) =
                (self.vocabulary.get(&word), self.idf_scores.get(&word))
            {
...
impl FeatureExtractor for ContentFeatureExtractor {
    fn extract_features(&self, text: &str) -> Vec<f32> {
        ...
        let lexical_diversity =
            if word_count > 0.0 { unique_words.len() as f32 / word_count } else { 0.0 };
        ...
        let punctuation_density =
            if char_count > 0.0 { punctuation_count / char_count } else { 0.0 };
...
impl SemanticEmbedder {
    ...
    pub fn new(dimension: usize) -> Self {
        let feature_extractors: Vec<Box<dyn FeatureExtractor>> =
            vec![Box::new(NamedEntityExtractor::new()), Box::new(ContentFeatureExtractor)];

        Self { dimension, feature_extractors }
    }
    ...
    pub fn with_extractors(dimension: usize, extractors: Vec<Box<dyn FeatureExtractor>>) -> Self {
        Self { dimension, feature_extractors: extractors }
    }
...
mod tests {
    ...
    async fn test_content_feature_extractor() {
        ...
        let features = extractor.extract_features(
            "This is a test. It has multiple sentences. Each sentence ends with a period.",
        );
