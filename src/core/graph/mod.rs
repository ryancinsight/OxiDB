...
impl GraphFactory {
    ...
    /// Returns a trait object that provides `GraphOperations`, `GraphQuery`, and `GraphTransaction`
    pub fn create_persistent_graph(
        path: impl AsRef<std::path::Path>,
    ) -> Result<Box<dyn storage::GraphStore>, OxidbError> {
...
mod tests {
    ...
    fn test_comprehensive_graph_store_capabilities() {
        ...
        let alice_nodes =
            graph.find_nodes_by_property("name", &Value::Text("Alice".to_string())).unwrap();
        ...
        let traversal_result =
            graph.traverse(node1_id, TraversalStrategy::BreadthFirst, Some(2)).unwrap();
