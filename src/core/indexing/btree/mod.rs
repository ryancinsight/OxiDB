...
use std::io::{Read, Seek}; // Added Seek and Read
...
use crate::core::query::commands::Value as TraitValue; // Value type from the trait // Common error type used by the Index trait
...
fn map_btree_error_to_common(btree_error: tree::OxidbError) -> CommonError {
    match btree_error {
        ...
        tree::OxidbError::Serialization(se) => {
            CommonError::Serialization(format!("BTree Serialization: {:?}", se))
        }
        tree::OxidbError::NodeNotFound(page_id) => {
        ...
        tree::OxidbError::UnexpectedNodeType => {
            CommonError::Index("BTree Unexpected Node Type".to_string())
        }
        tree::OxidbError::TreeLogicError(s) => {
...
impl Index for BPlusTreeIndex {
    ...
    fn load(&mut self) -> Result<(), CommonError> {
        let mut file = self.file_handle.lock().map_err(|e| {
            CommonError::Lock(format!("Failed to lock file handle mutably for load: {}", e))
        })?;
...
mod tests {
    ...
    use crate::core::query::commands::{Key as TestKey, Value as TestValue}; // Actual types for tests
