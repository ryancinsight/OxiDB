...
#[test]
    fn test_internal_borrow_from_right_sibling_new() -> Result<(), OxidbError> {
        ...
        match &il0_node_after {
            BPlusTreeNode::Internal { children, .. } => {
                assert_eq!(children.as_slice(), &[ML01_PID, L2_PID], "IL0 children incorrect after borrow");
            }
        ...
        match &il1_node_after {
            BPlusTreeNode::Internal { children, .. } => {
                assert_eq!(children.as_slice(), &[L3_PID, L4_PID], "IL1 children incorrect after borrow");
            }
...
#[test]
    fn test_internal_borrow_from_left_sibling_new() -> Result<(), OxidbError> {
        ...
        let nodes_to_construct = vec![
            BPlusTreeNode::Internal { page_id: R_PID, parent_page_id: None, keys: vec![k("05")], children: vec![IL0_PID, IL1_PID] },
            BPlusTreeNode::Internal { page_id: IL0_PID, parent_page_id: Some(R_PID), keys: vec![k("01"), k("03")], children: vec![L0_PID, L1_PID, L2_PID] },
            BPlusTreeNode::Leaf { page_id: L0_PID, parent_page_id: Some(IL0_PID), keys: vec![k("00")], values: vec![vec![pk("v00")]], next_leaf: Some(L1_PID) },
            BPlusTreeNode::Leaf { page_id: L1_PID, parent_page_id: Some(IL0_PID), keys: vec![k("01"), k("02")], values: vec![vec![pk("v01")], vec![pk("v02")]], next_leaf: Some(L2_PID) },
            BPlusTreeNode::Leaf { page_id: L2_PID, parent_page_id: Some(IL0_PID), keys: vec![k("03"), k("04")], values: vec![vec![pk("v03")], vec![pk("v04")]], next_leaf: Some(L3_PID) },
            BPlusTreeNode::Internal { page_id: IL1_PID, parent_page_id: Some(R_PID), keys: vec![k("07")], children: vec![L3_PID, L4_PID] },
            BPlusTreeNode::Leaf { page_id: L3_PID, parent_page_id: Some(IL1_PID), keys: vec![k("06")], values: vec![vec![pk("v06")]], next_leaf: Some(L4_PID) },
            BPlusTreeNode::Leaf { page_id: L4_PID, parent_page_id: Some(IL1_PID), keys: vec![k("07")], values: vec![vec![pk("v07")]], next_leaf: None },
        ];
        ...
        match &il0_node_after {
            BPlusTreeNode::Internal { children, .. } => {
                assert_eq!(children.as_slice(), &[L0_PID, L1_PID], "IL0 children incorrect after lending");
            }
            _ => panic!("IL0 should be internal"),
        }
        let il1_node_after = tree.read_node(IL1_PID)?;
        ...
        match &il1_node_after {
            BPlusTreeNode::Internal { children, .. } => {
                assert_eq!(children.as_slice(), &[L2_PID, ML34_PID], "IL1 children incorrect after borrowing");
            }
            _ => panic!("IL1 should be internal"),
        }
...
#[test]
    fn test_internal_merge_with_left_sibling_new() -> Result<(), OxidbError> {
        ...
        match ml23_node { Leaf { next_leaf, .. } => assert_eq!(next_leaf, Some(L4_PID)), _ => panic!() }; // Removed &

        let mut deallocated_pages_found = std::collections::HashSet::new();
        deallocated_pages_found.insert(tree.allocate_new_page_id()?);
        deallocated_pages_found.insert(tree.allocate_new_page_id()?);

        assert!(deallocated_pages_found.contains(&L3_PID));
        assert!(deallocated_pages_found.contains(&IL1_PID));
        Ok(())
    }
