...
impl BPlusTreeIndex {
    ...
    pub fn read_node(&self, page_id: PageId) -> Result<BPlusTreeNode, OxidbError> {
        let mut file = self.file_handle.lock().map_err(|e| OxidbError::BorrowError(format!("Mutex lock error: {}", e)))?;
        let offset = PAGE_SIZE + page_id * PAGE_SIZE;
    ...
    pub fn delete(&mut self, key_to_delete: &KeyType, pk_to_remove: Option<&PrimaryKey>) -> Result<bool, OxidbError> {
        ...
        let mut modification_occurred = false;
        match &mut leaf_node {
            BPlusTreeNode::Leaf { keys, values, .. } => {
    ...
    fn handle_underflow(&mut self, mut current_node: BPlusTreeNode, mut path: Vec<PageId>) -> Result<(), OxidbError> {
        ...
        if current_node_pid == self.root_page_id {
            if let BPlusTreeNode::Internal { ref keys, ref children, .. } = &current_node { // Changed to `ref keys`
        ...
        let child_idx_in_parent = match &parent_node {
            BPlusTreeNode::Internal { children, .. } => { // No need for `ref` here as `children` is copied by `iter()`
        ...
        if parent_node.get_keys().len() < self.min_keys_for_node() && parent_pid != self.root_page_id {
             ...
             if let BPlusTreeNode::Internal { ref children, .. } = parent_node { // Changed to `ref children`
                if children.len() == 1 {
                    self.root_page_id = children[0];
                    let mut new_root_node = self.read_node_mut(self.root_page_id)?;
                    new_root_node.set_parent_page_id(None);
        ...
        match (&mut *underflowed_node, &mut *lender_sibling, &mut *parent_node) {
            (
                ...
                if is_left_lender {
                    ...
                    if l_keys.is_empty() {
                         return Err(OxidbError::TreeLogicError("Lender leaf became empty after borrow; merge might be required.".to_string()));
                    }
                    p_keys[parent_key_idx] = l_keys[0].clone();
                }
            },
            (
                ...
                if is_left_lender {
                    ...
                    let new_separator_key_for_parent = l_keys.pop().ok_or(OxidbError::TreeLogicError("Lender internal node has no keys to pop".to_string()))?;
                    p_keys[parent_key_idx] = new_separator_key_for_parent;
                    ...
                    let new_separator_key_for_parent = l_keys.remove(0);
                    p_keys[parent_key_idx] = new_separator_key_for_parent;
...
mod tests {
    ...
    fn test_new_tree_creation() {
        let (mut tree, path, _dir) = setup_tree("test_new");
    ...
    fn test_node_read_write() {
        ...
        let read_node = tree.read_node(page_id1).unwrap();
        ...
        let read_internal_node = tree.read_node(page_id2).unwrap();
    ...
    fn test_delete_specific_pk_from_leaf() {
        ...
        let deleted_pk_result = tree.delete(&k("a"), Some(&pk("v_a2"))).unwrap();
        assert!(deleted_pk_result, "Deletion of a specific PK should return true if PK was found and removed.");
        let pks_a_after_delete = tree.find_primary_keys(&k("a")).unwrap().unwrap();
        ...
        let deleted_key_entirely = tree.delete(&k("a"), None).unwrap();
        assert!(deleted_key_entirely, "Deletion of entire key should return true.");
        assert!(tree.find_primary_keys(&k("a")).unwrap().is_none(), "Key 'a' should be completely gone.");
    }
    ...
    #[test]
    fn test_delete_leaf_borrow_from_left_sibling() {
        let (mut tree, _path, _dir) = setup_tree("delete_leaf_borrow_left");
        tree.root_page_id = 2; tree.next_available_page_id = 3; tree.write_metadata().unwrap();
    ...
    #[test]
    fn test_delete_internal_borrow_from_right_sibling() {
        ...
        tree.write_node(&p0).unwrap(); tree.write_node(&p2).unwrap(); tree.write_node(&p3).unwrap(); tree.write_node(&p4).unwrap();
        ...
        assert_eq!(l0_internal.get_keys(), &vec![k("E")], "L0 should get key 'E' from parent");
        assert_eq!(l0_internal.get_children().unwrap(), &vec![5, 7], "L0 should get child p2 (ID 7) from L1");
        let child_p2_updated = tree.read_node(7).unwrap();
        assert_eq!(child_p2_updated.get_parent_page_id(), Some(0), "Child p2's parent should be updated to L0 (ID 0)");
        assert_eq!(l1_internal.get_keys(), &vec![k("I")], "L1 should have key 'G' moved to parent, remaining key 'I'");
        assert_eq!(l1_internal.get_children().unwrap(), &vec![8, 9], "L1 children should be p3, p4");
        assert_eq!(root_internal.get_keys(), &vec![k("G")], "Root key should be updated to 'G' from L1");
    }

    #[test]
    fn test_delete_internal_borrow_from_left_sibling() {
        let (mut tree, _path, _dir) = setup_tree("delete_internal_borrow_left");
        tree.root_page_id = 4; tree.next_available_page_id = 10; tree.write_metadata().unwrap();
        let p0 = BPlusTreeNode::Leaf {page_id:5,parent_page_id:Some(0),keys:vec![k("A")],values:vec![vec![pk("vA")]],next_leaf:Some(6)};
        ...
        assert_eq!(l1_internal.get_keys(), &vec![k("G")], "L1 should get key 'G' from parent");
        assert_eq!(l1_internal.get_children().unwrap(), &vec![7, 8], "L1 should get child p2 (ID 7) from L0");
        let child_p2_updated = tree.read_node(7).unwrap();
        assert_eq!(child_p2_updated.get_parent_page_id(), Some(1), "Child p2's parent should be updated to L1 (ID 1)");
        assert_eq!(l0_internal.get_keys(), &vec![k("B")], "L0 should have key 'D' moved to parent, remaining 'B'");
        assert_eq!(l0_internal.get_children().unwrap(), &vec![5, 6], "L0 children should be p0, p1");
        assert_eq!(root_internal.get_keys(), &vec![k("D")], "Root key should be updated to 'D' from L0");
    }

    #[test]
    fn test_delete_leaf_merge_with_right_sibling() {
        let (mut tree, _path, _dir) = setup_tree("delete_leaf_merge_right");
        tree.root_page_id = 2; tree.next_available_page_id = 3; tree.write_metadata().unwrap();
        let l0 = BPlusTreeNode::Leaf { page_id:0, parent_page_id:Some(2), keys:vec![k("A")], values:vec![vec![pk("vA")]], next_leaf:Some(1) }; // L0 has 1 key, will underflow
        let l1 = BPlusTreeNode::Leaf { page_id:1, parent_page_id:Some(2), keys:vec![k("D")], values:vec![vec![pk("vD")]], next_leaf:None };   // L1 has 1 key, cannot lend
        let r = BPlusTreeNode::Internal { page_id:2, parent_page_id:None, keys:vec![k("C")], children:vec![0,1] }; // Parent key "C"
        tree.write_node(&l0).unwrap(); tree.write_node(&l1).unwrap(); tree.write_node(&r).unwrap();

        assert!(tree.delete(&k("A"), None).unwrap()); // Delete "A" from L0, L0 is now empty and underflows
                                                  // Expected: L0 merges with L1. Parent "C" removed. Root becomes L0. L0 has ["C","D"] (error in old comment)
                                                  // Correct Leaf Merge: L0 absorbs L1. L0 keys: ["D"]. Parent key "C" is removed. Root becomes L0.

        let final_root_node = tree.read_node(tree.root_page_id).unwrap();
        assert_eq!(tree.root_page_id, 0, "Root should become L0 after merge and root underflow.");
        if let BPlusTreeNode::Leaf { page_id, keys, values, next_leaf, parent_page_id } = final_root_node {
            assert_eq!(page_id, 0);
            assert!(parent_page_id.is_none());
            assert_eq!(keys, vec![k("D")]);
            assert_eq!(values, vec![vec![pk("vD")]]);
    ...
    #[test]
    fn test_delete_leaf_merge_with_left_sibling() {
        let (mut tree, _path, _dir) = setup_tree("delete_leaf_merge_left");
        tree.root_page_id = 2; tree.next_available_page_id = 3; tree.write_metadata().unwrap();
        let l0_orig = BPlusTreeNode::Leaf { page_id:0, parent_page_id:Some(2), keys:vec![k("A")], values:vec![vec![pk("vA")]], next_leaf:Some(1) }; // L0 has 1 key, cannot lend
        let l1_orig = BPlusTreeNode::Leaf { page_id:1, parent_page_id:Some(2), keys:vec![k("C")], values:vec![vec![pk("vC")]], next_leaf:None };   // L1 has 1 key, will underflow
        let r_orig = BPlusTreeNode::Internal { page_id:2, parent_page_id:None, keys:vec![k("B")], children:vec![0,1] }; // Parent key "B"
        tree.write_node(&l0_orig).unwrap(); tree.write_node(&l1_orig).unwrap(); tree.write_node(&r_orig).unwrap();

        assert!(tree.delete(&k("C"), None).unwrap()); // Delete "C" from L1, L1 is now empty and underflows
                                                  // Expected: L0 merges L1. Parent "B" removed. Root becomes L0. L0 has ["A", "B"].
                                                  // Correct Leaf Merge: L0 absorbs L1. L0 keys: ["A"]. Parent key "B" removed. Root becomes L0.
    ...
    #[test]
    fn test_delete_internal_node_merge() {
        let (mut tree, _path, _dir) = setup_tree("delete_internal_merge");
        ...
        tree.write_node(&l0).unwrap(); tree.write_node(&l1).unwrap(); tree.write_node(&l2).unwrap();
        ...
        let mut p1_for_merge = tree.read_node_mut(0).unwrap();
        let mut p2_for_merge = tree.read_node_mut(1).unwrap();
        let mut gp_for_merge = tree.read_node_mut(6).unwrap();
        ...
        assert_eq!(tree.root_page_id, 0, "Root page ID should be P1's page ID (0)");
        let final_root_node = tree.read_node(tree.root_page_id).unwrap();
        if let BPlusTreeNode::Internal { page_id, keys, children, parent_page_id } = final_root_node {
            ...
            let child_l2_updated = tree.read_node(4).unwrap();
