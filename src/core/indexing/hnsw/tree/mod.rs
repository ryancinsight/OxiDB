...
impl HnswIndex {
    ...
    pub fn new(
        ...
        Ok(Self { name, graph, pk_to_node: HashMap::new(), dimension })
    }

    /// Parse a vector value from bytes
    pub fn parse_vector_value(
        &self,
        value: &TraitValue,
    ...
    pub fn insert_vector(&mut self, vector: Vector, primary_key: PrimaryKey) -> HnswResult<()> {
        ...
        if self.pk_to_node.contains_key(&primary_key) {
            return Err(HnswError::Generic("Primary key already exists in index".to_string()));
        }
