...
impl DeadlockDetector {
    fn new() -> Self {
        Self { wait_for_graph: HashMap::new() }
    }

    fn add_wait_for(&mut self, waiting_tx: TransactionId, holding_tx: TransactionId) {
        self.wait_for_graph.entry(waiting_tx).or_default().insert(holding_tx);
    }
    ...
    fn detect_cycle(&self) -> Option<Vec<TransactionId>> {
        for &start_tx in self.wait_for_graph.keys() {
            if let Some(cycle) =
                self.dfs_cycle_detection(start_tx, &mut HashSet::new(), &mut Vec::new())
            {
                return Some(cycle);
            }
        }
...
impl AcidTransactionManager {
    ...
    pub fn begin_transaction(
    ...
    ) -> Result<TransactionId, OxidbError> {
        ...
        let _begin_record = LogRecord::BeginTransaction { lsn, tx_id };
    ...
    pub fn acquire_lock(
            ...
            if holder != tx_id.0 {
                ...
                if let Some(cycle) = deadlock_detector.detect_cycle() {
                    ...
                    return Err(OxidbError::DeadlockDetected(format!(
                        "Transaction {victim:?} aborted due to deadlock"
                    )));
                }
            }
        }
        ...
        if lock_manager.acquire_lock(tx_id.0, &key_bytes, lock_mode).is_ok() {
            ...
            Err(OxidbError::LockTimeout(format!(
