...
impl<'a> BorrowedPredicate<'a> {
    ...
    pub fn new(column_index: usize, operator: ComparisonOp, value: BorrowedValue<'a>) -> Self {
        Self { column_index, operator, value }
    }
    ...
    pub fn evaluate(&self, row: &Row) -> bool {
        ...
        match (&self.value, column_value) {
            ...
            (BorrowedValue::Text(a), Value::Text(b)) => match self.operator {
            ...
            (BorrowedValue::Boolean(a), Value::Boolean(b)) => match self.operator {
                ComparisonOp::Equal => b == a,
                ComparisonOp::NotEqual => b != a,
                _ => false,
            },
...
mod tests {
    ...
    fn test_borrowed_query_plan() {
        let plan = BorrowedQueryPlan::new(BorrowedPlanNode::Filter {
    ...
    fn test_pattern_matching() {
        // Test exact matches
        let pred =
            BorrowedPredicate::new(0, ComparisonOp::Like, BorrowedValue::Text("hello".into()));
    ...
    fn test_like_predicate_evaluation() {
        let pred =
            BorrowedPredicate::new(0, ComparisonOp::Like, BorrowedValue::Text("John%".into()));
        ...
        // Test NOT LIKE
        let not_pred =
            BorrowedPredicate::new(0, ComparisonOp::NotLike, BorrowedValue::Text("John%".into()));
