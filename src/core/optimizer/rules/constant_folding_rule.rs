...
impl ConstantFoldingRule {
    ...
    fn evaluate_comparison(
        &self,
        comp: &crate::core::query::sql::ast::Condition,
    ) -> Result<Option<bool>, OxidbError> {
        // Handle tautologies and contradictions
        match (&comp.operator[..], &comp.value) {
            // Always true conditions
            ("=", AstExpressionValue::Literal(AstLiteralValue::Boolean(true)))
                if comp.column == "true" =>
            {
                Ok(Some(true))
            }
            ("!=", AstExpressionValue::Literal(AstLiteralValue::Boolean(false)))
                if comp.column == "true" =>
            {
                Ok(Some(true))
            }
        ...
        if let AstExpressionValue::Literal(AstLiteralValue::Number(num_str)) = &comp.value {
            if let Ok(parsed) = num_str.parse::<f64>() {
                ...
                if parsed.fract() == 0.0 && parsed.abs() <= i64::MAX as f64 {
                    simplified.value = AstExpressionValue::Literal(AstLiteralValue::Number(
...
mod tests {
    ...
    fn test_double_negation_elimination() {
        ...
        let double_negated =
            ConditionTree::Not(Box::new(ConditionTree::Not(Box::new(original_condition.clone()))));
