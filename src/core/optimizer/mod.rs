...
impl Optimizer {
    ...
    fn convert_simple_condition_to_expression(&self, condition: &crate::core::query::sql::ast::ConditionTree) -> Result<Expression, crate::core::common::error::OxidbError> {
        use crate::core::query::sql::ast::{ConditionTree, AstExpressionValue, AstLiteralValue};
                ...
                match &cond.value {
                    AstExpressionValue::Literal(literal_value) => {
                        ...
                        let data_type = match literal_value {
                            ...
                            AstLiteralValue::String(s) => crate::core::types::DataType::String(s.clone()),
    ...
    pub fn optimize_with_indexes(&self, plan: QueryPlanNode, index_manager: &std::sync::Arc<std::sync::RwLock<crate::core::indexing::manager::IndexManager>>) -> Result<QueryPlanNode, crate::core::common::error::OxidbError> {
    ...
    fn apply_index_selection(&self, plan: QueryPlanNode, index_manager: &std::sync::Arc<std::sync::RwLock<crate::core::indexing::manager::IndexManager>>) -> Result<QueryPlanNode, crate::core::common::error::OxidbError> {
        match plan {
            ...
            QueryPlanNode::Filter { input, predicate } => {
                if let QueryPlanNode::TableScan { table_name, alias } = *input {
                    ...
                    if let Some(index_scan) = self.try_convert_to_index_scan(&table_name, &predicate, alias.clone(), index_manager)? {
                        Ok(index_scan)
                    } else {
    ...
    fn try_convert_to_index_scan(
        ...
        alias: Option<String>,
        index_manager: &std::sync::Arc<std::sync::RwLock<crate::core::indexing::manager::IndexManager>>
    ...
    fn extract_indexable_predicate(&self, expr: &Expression) -> Option<SimplePredicate> {
        match expr {
            Expression::CompareOp { left, op, right } if op == "=" => {
                ...
                if let (Expression::Column(column), Expression::Literal(value)) = (left.as_ref(), right.as_ref()) {
                ...
                if let (Expression::Literal(value), Expression::Column(column)) = (left.as_ref(), right.as_ref()) {
                    return Some(SimplePredicate {
                        column: column.clone(),
                        operator: op.clone(),
                        value: value.clone(),
