...
impl VectorTransaction {
    ...
    pub fn commit(&mut self) -> Result<(), OxidbError> {
        if self.state != TransactionState::Active {
            return Err(OxidbError::Transaction(
                "Cannot commit inactive transaction".to_string(),
            ));
        }
...
impl VectorTransactionManager {
    ...
    pub fn begin_transaction(&self) -> Result<TransactionId, OxidbError> {
        let mut next_id = self.next_tx_id.lock().map_err(|_| {
            OxidbError::Lock("Failed to acquire next_tx_id lock".to_string())
        })?;
    ...
    pub fn transactional_store(
        ...
        let transaction = active_txs.get_mut(&tx_id).ok_or_else(|| {
            OxidbError::Transaction(format!("Transaction {} not found", tx_id))
        })?;
        ...
        let mut store = self.store.lock().map_err(|_| {
            OxidbError::Lock("Failed to acquire store lock".to_string())
        })?;
        
        let existing_entry = store.retrieve(&id)?;
        if let Some(existing) = existing_entry {
            transaction.add_undo_operation(UndoOperation::Restore {
                id: id.clone(),
                entry: existing,
            });
        } else {
    ...
    pub fn transactional_delete(
        ...
        if let Some(existing) = existing_entry {
            ...
            // Log the operation
            transaction.add_operation(VectorOperation::Delete {
                id: id.to_string(),
            })?;
    ...
    pub fn commit_transaction(&self, tx_id: TransactionId) -> Result<(), OxidbError> {
        ...
        let mut transaction = active_txs.remove(&tx_id).ok_or_else(|| {
            OxidbError::Transaction(format!("Transaction {} not found", tx_id))
        })?;
...
mod tests {
    use super::*;
    use crate::core::vector::{VectorFactory, storage::InMemoryVectorStore};
