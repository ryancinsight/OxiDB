...
8.  **Implement Query Planner and Optimizer:**
    ...
    *   [~] **Validation:** Optimized physical query plans are generated. (Project node now uses BoundExpressions with correctly resolved column types for SELECTs; overall optimization state improved, but further work on schema propagation for joins and other contexts pending).
...
9.  **Implement Query Execution Engine (Volcano/Iterator Model):**
    *   [~] Implement executor for each physical plan operator.
        ...
        *   [x] Subtask: Each operator should implement `get_output_schema()` method. (Added to trait and implemented for TableScan, Filter, Project, NestedLoopJoin).
            *   [x] Sub-subtask: Implement `TableScanExecutor` to read rows from a table. (Exists, `get_output_schema` added, deserialization refined for nullability).
        ...
        *   [~] Subtask: Write unit tests for each individual operator.
            *   [x] Sub-subtask: Test `TableScanExecutor` by reading all rows from a known table. (Covered by API tests using SELECT * or similar, SELECT * functionality improved).
            *   [x] Sub-subtask: Test `FilterExecutor` with various predicates. (Covered by API tests using WHERE clauses).
            *   [~] Sub-subtask: Test `ProjectOperator` with different column selections and expressions. (New expression evaluation tested at `expression_evaluator` level. Old `ProjectOperator` tests commented out; new operator-level tests for projection need `MockOperator` or similar harness. `SELECT *` and column-based vector functions now covered by API tests).
    *   [x] Implement core expression evaluation system (`core::execution::expression_evaluator`).
        ...
        *   [x] Subtask: Executes `COSINE_SIMILARITY` and `DOT_PRODUCT` via SQL (with literal arguments confirmed, and now column arguments also confirmed).
    ...
    *   [~] **Validation:** Queries are executed correctly, and results match expectations. (Vector function execution with column arguments now works. Overall query correctness improved due to better schema handling in projections.)
