...
fn test_basic_connection() -> Result<(), OxidbError> {
    ...
    let sql =
        format!("INSERT INTO users (id, name, active) VALUES ({}, 'Test User', true)", unique_id);
...
fn test_data_types() -> Result<(), OxidbError> {
    ...
    let insert_sql = format!(
        "INSERT INTO {} (id, text_field, integer_field, float_field, boolean_field) 
    ...
    let insert_sql = format!(
        "INSERT INTO {} (id, text_field, integer_field, float_field, boolean_field) 
                      VALUES (2, 'Test String', -100, 2.718, false)",
        table_name
    );
    conn.execute(&insert_sql)?;
    ...
    match result {
        QueryResult::Data(data) => {
            println!("✓ Retrieved {} records with columns: {:?}", data.row_count(), data.columns());
...
fn test_crud_operations() -> Result<(), OxidbError> {
    ...
    // Create
    conn.execute(
        "CREATE TABLE products (id INTEGER PRIMARY KEY, name TEXT, price FLOAT, in_stock BOOLEAN)",
    )?;
...
fn test_complex_queries() -> Result<(), OxidbError> {
    ...
    conn.execute(
        "CREATE TABLE items (id INTEGER PRIMARY KEY, name TEXT, category_id INTEGER, price FLOAT)",
    )?;
    ...
    conn.execute(
        "INSERT INTO items (id, name, category_id, price) VALUES (1, 'Laptop', 1, 999.99)",
    )?;
    conn.execute(
        "INSERT INTO items (id, name, category_id, price) VALUES (2, 'Phone', 1, 599.99)",
    )?;
    conn.execute("INSERT INTO items (id, name, category_id, price) VALUES (3, 'Novel', 2, 19.99)")?;
    conn.execute(
        "INSERT INTO items (id, name, category_id, price) VALUES (4, 'Textbook', 2, 89.99)",
    )?;
    conn.execute(
        "INSERT INTO items (id, name, category_id, price) VALUES (5, 'T-Shirt', 3, 24.99)",
    )?;
