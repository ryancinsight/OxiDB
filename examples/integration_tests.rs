...
fn test_basic_crud(process_id: u32) -> Result<(), OxidbError> {
    ...
    // Create
    let sql =
        format!("CREATE TABLE {} (id INTEGER PRIMARY KEY, name TEXT, price FLOAT)", table_name);
    conn.execute(&sql)?;
...
fn test_data_types(process_id: u32) -> Result<(), OxidbError> {
    ...
    let sql = format!(
        "INSERT INTO {} (id, text_val, int_val, float_val, bool_val) 
                      VALUES (1, 'Hello', 42, 3.14, true)",
        table_name
    );
    conn.execute(&sql)?;
    ...
    let sql = format!(
        "INSERT INTO {} (id, text_val, int_val, float_val, bool_val) 
                      VALUES (2, 'World', -100, 2.718, false)",
        table_name
    );
    conn.execute(&sql)?;
