...
fn perform_crud_test() -> Result<(), OxidbError> {
    ...
    // Create
    conn.execute(
        "CREATE TABLE validation_test (id INTEGER PRIMARY KEY, name TEXT, value INTEGER)",
    )?;
...
fn perform_data_type_test() -> Result<(), OxidbError> {
    ...
    conn.execute(
        "INSERT INTO type_validation (id, text_col, int_col, float_col, bool_col) 
...
fn perform_concurrent_test() -> Result<(), OxidbError> {
    ...
    for i in 1..=10 {
        conn.execute(&format!(
            "INSERT INTO concurrent_test (id, data) VALUES ({}, 'Data {}')",
            i, i
        ))?;
    }
...
fn perform_performance_test() -> Result<(), OxidbError> {
    ...
    for i in 1..=50 {
        conn.execute(&format!(
            "INSERT INTO perf_test (id, data) VALUES ({}, 'Performance Test {}')",
            i, i
        ))?;
    }
...
fn perform_scalability_test() -> Result<(), OxidbError> {
    ...
    for i in 1..=25 {
        conn.execute(&format!(
            "INSERT INTO scale_test (id, data) VALUES ({}, 'Scale Test {}')",
            i, i
        ))?;
    }
...
impl TestResult {
    fn new(name: &str, passed: bool, duration: std::time::Duration, error: Option<String>) -> Self {
        Self { name: name.to_string(), passed, duration, error }
    }
}
