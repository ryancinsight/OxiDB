...
fn test_error_handling() -> Result<(), OxidbError> {
    ...
    match conn.execute("SELECT * FROM non_existent_table") {
        Err(_) => {} // Expected to fail
        Ok(_) => {
            return Err(OxidbError::Other("Should have failed on non-existent table".to_string()))
        }
    }
...
fn test_core_functionality() -> Result<(), OxidbError> {
    ...
    // Test CREATE TABLE
    let create_sql =
        format!("CREATE TABLE {} (id INTEGER PRIMARY KEY, name TEXT, value INTEGER)", table_name);
    conn.execute(&create_sql)?;

    // Test INSERT
    let insert_sql =
        format!("INSERT INTO {} (id, name, value) VALUES (1, 'Alice', 100)", table_name);
    conn.execute(&insert_sql)?;
...
fn test_performance() -> Result<(), OxidbError> {
    ...
    for i in 1..=100 {
        let insert_sql =
            format!("INSERT INTO {} (id, data) VALUES ({}, 'data_{}')", table_name, i, i);
        conn.execute(&insert_sql)?;
    }
    ...
    println!(
        "   📊 Inserted 100 records in {:?} ({:.2} records/sec)",
        duration,
...
fn test_transaction_management() -> Result<(), OxidbError> {
    ...
    // Create table
    let create_sql =
        format!("CREATE TABLE {} (id INTEGER PRIMARY KEY, balance INTEGER)", table_name);
    conn.execute(&create_sql)?;
