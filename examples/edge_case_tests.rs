...
fn test_boundary_values() -> Result<(), OxidbError> {
    ...
    conn.execute(
        "CREATE TABLE boundary_test (id INTEGER PRIMARY KEY, value TEXT, number INTEGER)",
    )?;
    ...
    conn.execute(&format!(
        "INSERT INTO boundary_test (id, value, number) VALUES (2, '{}', 999999)",
        long_string
    ))?;
...
fn test_data_type_edge_cases() -> Result<(), OxidbError> {
    ...
    conn.execute(
        "INSERT INTO data_test (id, text_field, int_field, float_field, bool_field) 
...
fn test_constraint_validation() -> Result<(), OxidbError> {
    ...
    conn.execute(
        "INSERT INTO constraint_test (id, email, age) VALUES (1, 'user@example.com', 25)",
    )?;

    // Test unique constraint
    match conn
        .execute("INSERT INTO constraint_test (id, email, age) VALUES (2, 'user@example.com', 30)")
    {
...
fn test_performance_edge_cases() -> Result<(), OxidbError> {
    ...
    for i in 1..=100 {
        conn.execute(&format!(
            "INSERT INTO perf_test (id, data) VALUES ({}, 'Test data {}')",
            i, i
        ))?;
    }
...
fn test_concurrency_simulation() -> Result<(), OxidbError> {
    ...
    conn.execute(
        "CREATE TABLE concurrent_test (id INTEGER PRIMARY KEY, thread_id INTEGER, timestamp TEXT)",
    )?;
