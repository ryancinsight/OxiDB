...
fn test_integer_boundaries(conn: &mut Connection) -> Result<(), OxidbError> {
    ...
    let test_values = vec![i64::MAX, i64::MIN, 0, -1, 1, i32::MAX as i64, i32::MIN as i64];
...
fn test_string_boundaries(conn: &mut Connection) -> Result<(), OxidbError> {
    ...
    for (text, description) in test_cases {
        let sql =
            format!("INSERT INTO string_bounds (text_col) VALUES ('{}')", escape_sql_string(text));
...
fn test_type_coercion_edge_cases(conn: &mut Connection) -> Result<(), OxidbError> {
    ...
    let test_cases = vec![
        ...
        (
            "INSERT INTO coercion_test (int_col) VALUES ('not_a_number')",
    ...
    for (sql, description) in test_cases {
        match conn.execute(sql) {
            ...
            Err(e) => {
                println!("⚠ Type coercion failed (may be expected): {} - {:?}", description, e)
            }
        }
    }
...
fn test_memory_pressure_scenarios() -> Result<(), OxidbError> {
    ...
    for i in 1..=10 {
        ...
        match conn
            .execute(&format!("INSERT INTO memory_pressure_test (data) VALUES (x'{}')", hex_data))
        {
...
fn run_sql_injection_protection_tests() -> Result<(), OxidbError> {
    ...
    let mut conn = Connection::open_in_memory()?;
    conn.execute(
        "CREATE TABLE injection_test (id INTEGER PRIMARY KEY, username TEXT, password TEXT)",
    )?;
...
fn test_corrupted_data_handling() -> Result<(), OxidbError> {
    ...
    match Connection::open(db_path) {
        Ok(mut conn) => match conn.execute("SELECT * FROM corruption_test") {
