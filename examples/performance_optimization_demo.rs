...
fn setup_test_schema(conn: &mut Connection) -> Result<(), OxidbError> {
    ...
    for i in 1..=1000 {
        let vector_str =
            (0..128).map(|_| rng.gen_range(0.0..1.0).to_string()).collect::<Vec<_>>().join(", ");
...
fn run_unoptimized_workload(conn: &mut Connection) -> Result<WorkloadMetrics, OxidbError> {
    ...
    for user_id in [42, 123, 456, 789, 999] {
        ...
        conn.execute(&format!("SELECT * FROM posts WHERE user_id = {}", user_id))?;
    ...
    for post_id in [100, 500, 1000, 2500, 4999] {
        let query_start = Instant::now();
        conn.execute(&format!("SELECT * FROM comments WHERE post_id = {}", post_id))?;
        query_times.push(query_start.elapsed());
    }
...
fn compare_performance(unoptimized: &WorkloadMetrics, optimized: &WorkloadMetrics) {
    ...
    let query_speedup =
        unoptimized.avg_query_time.as_secs_f64() / optimized.avg_query_time.as_secs_f64();
    ...
    println!("├─────────────────────────────────────────────┤");
    println!(
        "│ Total Duration      │ {:>8.2}s │ {:>8.2}s │",
        unoptimized.total_duration.as_secs_f64(),
        optimized.total_duration.as_secs_f64()
    );
    println!(
        "│ Avg Query Time      │ {:>8.2}ms│ {:>8.2}ms│",
        unoptimized.avg_query_time.as_millis(),
        optimized.avg_query_time.as_millis()
    );
    println!(
        "│ Queries/Second      │ {:>9.1} │ {:>9.1} │",
        unoptimized.queries_per_second, optimized.queries_per_second
    );
    println!(
        "│ Cache Hit Rate      │ {:>8.1}% │ {:>8.1}% │",
...
fn demonstrate_advanced_optimizations(conn: &mut Connection) -> Result<(), OxidbError> {
    ...
    let query_vector =
        (0..128).map(|i| (i as f32 * 0.01).to_string()).collect::<Vec<_>>().join(", ");
