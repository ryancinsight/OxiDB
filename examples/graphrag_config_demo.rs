...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let graph_store: Arc<Mutex<Box<dyn GraphStore>>> =
        Arc::new(Mutex::new(GraphFactory::create_memory_graph()?));

    // Example 1: Using default configuration
    println!("1. Default Configuration:");
    let default_embedder: Arc<dyn EmbeddingModel + Send + Sync> =
        Arc::new(SemanticEmbedder::new(384));
    let _engine1 = GraphRAGEngineImpl::new(
        graph_store.clone(),
    ...
    println!(
        ...
        GraphRAGConfig::default().default_similarity_threshold
    );
    ...
    let _engine2 =
        GraphRAGEngineImpl::new(graph_store.clone(), default_embedder.clone(), config.clone());
    ...
    let tfidf_embedder: Arc<dyn EmbeddingModel + Send + Sync> =
        Arc::new(TfIdfEmbedder::new(&documents));
    let _engine3 =
        GraphRAGEngineImpl::new(graph_store.clone(), tfidf_embedder, GraphRAGConfig::default());
    ...
    let semantic_embedder: Arc<dyn EmbeddingModel + Send + Sync> =
        Arc::new(SemanticEmbedder::new(1024));
    let _engine4 =
        GraphRAGEngineImpl::new(graph_store, semantic_embedder, GraphRAGConfig::default());
