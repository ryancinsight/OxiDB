...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    conn.execute("CREATE TABLE orders (id INTEGER PRIMARY KEY, user_id INTEGER, product TEXT, amount FLOAT)")?;
    ...
    println!("  • Slow Queries Detected: {}", if report.query_analysis.slow_queries_detected { "Yes" } else { "No" });
