use oxidb::{Connection, QueryResult, OxidbError};
...
fn main() -> Result<(), OxidbError> {
    ...
    let create_sql = format!("CREATE TABLE {} (id INTEGER PRIMARY KEY, name TEXT, price FLOAT)", products_table);
    file_conn.execute(&create_sql)?;
    
    let insert_sql = format!("INSERT INTO {} (id, name, price) VALUES (1, 'Laptop', 999.99)", products_table);
    file_conn.execute(&insert_sql)?;
    
    let insert_sql = format!("INSERT INTO {} (id, name, price) VALUES (2, 'Mouse', 29.99)", products_table);
    file_conn.execute(&insert_sql)?;
