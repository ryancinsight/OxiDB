...
fn benchmark_bulk_insert() -> Result<(), OxidbError> {
    ...
    for &count in &record_counts {
        ...
        println!(
            "✓ Inserted {} records in {:?} ({:.0} records/sec)",
...
fn benchmark_query_performance() -> Result<(), OxidbError> {
    ...
    let queries = [
        ...
        (
            "Complex Filter",
            "SELECT * FROM query_test WHERE category = 'B' AND score > 80 AND active = true",
        ),
    ...
    for (name, sql) in &queries {
        ...
        let count = match result {
            oxidb::QueryResult::Data(ds) => ds.row_count(),
            oxidb::QueryResult::RowsAffected(n) => n as usize,
            oxidb::QueryResult::Success => 0,
        };
...
fn benchmark_transaction_performance() -> Result<(), OxidbError> {
    ...
    for &count in &tx_counts {
        ...
        println!("✓ Completed {} transactions in {:?} ({:.0} tx/sec)", count, duration, tx_per_sec);
    }
...
fn benchmark_concurrent_operations() -> Result<(), OxidbError> {
    ...
    println!(
        "✓ Simulated {} concurrent operations ({} threads × {} ops) in {:?}",
