...
pub async fn login_user(req: LoginRequest) -> Result<LoginResponse> {
    ...
    if let QueryResult::Data(data) = result {
        if let Some(row) = data.rows.first() {
            ...
            let username =
                row.get(1).and_then(value_as_text).ok_or(anyhow!("Invalid user data"))?;
            let email = row.get(2).and_then(value_as_text).ok_or(anyhow!("Invalid user data"))?;
            let password_hash =
                row.get(3).and_then(value_as_text).ok_or(anyhow!("Invalid user data"))?;
            let created_at =
                row.get(4).and_then(value_as_text).ok_or(anyhow!("Invalid user data"))?;
            let updated_at =
                row.get(5).and_then(value_as_text).ok_or(anyhow!("Invalid user data"))?;
...
pub fn create_jwt_token(user_id: &str) -> Result<String> {
    ...
    let claims = Claims { sub: user_id.to_string(), exp: exp.timestamp(), iat: now.timestamp() };
